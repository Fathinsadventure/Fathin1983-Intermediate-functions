...
function getEmailDomain(email) {
    const monkeyTail = email.indexOf("@");
    const doMain = email.substring(monkeyTail + 1);
...
const typeA = typeOfEmail("n.eeken@novi-education.nl");
const typeB = typeOfEmail("t.mellink@novi.nl");
const typeC = typeOfEmail("novi.nlaapjesk@outlook.com");
...
function checkEmailValidity(email) {
    ...
    if (containsMonkeyTail && !containsComma && containsNoDotAtEnd) {
        return true;
    } else {
        return false;
    }
...
const valid2 = checkEmailValidity("tessmellink2novi.nl");
const valid3 = checkEmailValidity("n.eekenanovi.nl");
const valid4 = checkEmailValidity("tessmellink@novi,nl");
